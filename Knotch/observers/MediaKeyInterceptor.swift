    ...
    private func showHUD(for keyType: NXKeyType, command: Bool) {
        Task { @MainActor in
            ...
            case .soundUp, .soundDown, .mute:
                ...
                KnotchViewCoordinator.shared.toggleSneakPeek(status: true, type: .volume, value: CGFloat(v))
            case .brightnessUp, .brightnessDown:
                if command {
                    ...
                    KnotchViewCoordinator.shared.toggleSneakPeek(status: true, type: .brightness, value: CGFloat(v))
                }
